...
from src.models_new import MNL, MPMVSurrogate
...
if __name__ == "__main__":
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for line in tqdm(f, total=total_instances, desc="Redo SP"):
            ...
            time_1 = time.time()
            ro = RO(N)
            x_ro, val_ro = ro.maximize(mnl, C=(0, C), r=r)
            time_2 = time.time()
            x_milp = mnl.solve(C=C)
            time_3 = time.time()
            u = np.log(pf).tolist()
            sp = MPMVSurrogate(u, r, [0.0], 1, GumBel(), (0, C), solver_params=solver_params)
            x_sp = sp.solve(method='SP', n_steps=1001)
            time_4 = time.time()
